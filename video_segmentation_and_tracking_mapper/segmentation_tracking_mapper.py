...
class RuntimeContext:
    ...
    def sam3(self, args: argparse.Namespace):
        ...
        if (
            ...
            if args.sam3_use_unified_builder:
                try:
                    from sam3.model_builder import build_sam3_predictor
                except ImportError as exc:
                    raise RuntimeError("This SAM3 install does not expose build_sam3_predictor.") from exc
            ...
            if "version" in signature.parameters:
                kwargs["version"] = args.sam3_version
            if "gpus_to_use" in signature.parameters and self.device.startswith("cuda"):
                kwargs["gpus_to_use"] = range(self.torch.cuda.device_count())
            if "compile" in signature.parameters:
                kwargs["compile"] = args.sam3_compile
            if "warm_up" in signature.parameters:
                kwargs["warm_up"] = args.sam3_warm_up
            if "async_loading_frames" in signature.parameters:
                kwargs["async_loading_frames"] = args.sam3_async_loading_frames
            predictor = builder(**kwargs)
            self._sam3 = (predictor, kwargs, args.sam3_model, checkpoint_key, self.device)
...
def sam3_response_outputs(response: Any) -> dict[str, Any]:
...
def sam3_mask_from_outputs(np, torch, outputs: dict[str, Any], obj_id: int) -> Any | None:
    masks = tensor_to_numpy(np, torch, first_present(outputs, ("out_binary_masks", "pred_masks", "masks")))
    if masks is None:
        return None
    ...
    obj_ids = first_present(outputs, ("out_obj_ids", "obj_ids", "object_ids"))
    if obj_ids is not None:
        obj_ids = tensor_to_numpy(np, torch, obj_ids)
        obj_ids = [int(item) for item in np.asarray(obj_ids).reshape(-1).tolist()]
...
def track_with_sam3_prompt(
    ...
    if not hasattr(predictor, "handle_request"):
        raise RuntimeError("SAM3 predictor does not expose handle_request; cannot run video point prompting.")

    norm_point = [
        max(0.0, min(float(input_point[0]) / max(1.0, float(frame_width)), 1.0)),
        max(0.0, min(float(input_point[1]) / max(1.0, float(frame_height)), 1.0)),
    ]
    points_tensor = torch.tensor([norm_point], dtype=torch.float32)
    ...
    try:
        ...
        add_outputs = sam3_response_outputs(add_response)
        add_mask = sam3_mask_from_outputs(np, torch, add_outputs, obj_id)
        if add_mask is not None:
            frame_records.append(
                {
                    "frame_index": 0,
                    "mask": add_mask,
                    "bbox_xyxy": base.mask_to_bbox(np, add_mask),
                }
            )
        ...
        for fallback_frame_idx, response in enumerate(stream):
            outputs = sam3_response_outputs(response)
            frame_idx = int(first_present(outputs, ("frame_index", "frame_idx")) or fallback_frame_idx)
            if frame_idx in seen:
                continue
            mask = sam3_mask_from_outputs(np, torch, outputs, obj_id)
            if mask is None:
                continue
            frame_records.append(
                {
...
def run_pipeline(args: argparse.Namespace, context: RuntimeContext | None = None) -> dict[str, Any]:
    ...
    if args.sam_backend == "sam2":
        ...
        sam3_predictor, _sam3_kwargs = context.sam3(args)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--sam3-version", default="3.1")
    parser.add_argument("--sam3-obj-id", type=int, default=0)
    parser.add_argument("--sam3-compile", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--sam3-warm-up", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--sam3-async-loading-frames", action=argparse.BooleanOptionalAction, default=False)
    parser.add_argument("--sam3-use-unified-builder", action=argparse.BooleanOptionalAction, default=False)
